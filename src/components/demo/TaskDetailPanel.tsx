...
export function TaskDetailPanel({
  ...
  const resolvedEvent = event ?? findEventForTask(taskId);
  const taskKey = `${app ?? "_"}:${resolvedEvent ?? "_"}:${taskId}`;

  const task = useMemo(
    () => (resolvedEvent ? loadTaskFromTemplates(resolvedEvent, taskId) : null),
    [resolvedEvent, taskId],
  );
  ...
  const loadActivity = async () => {
    ...
    if (error) {
      ...
      const last = rows
        ...
        .pop();
      setStatus(last?.kind === "closed" ? "closed" : "open");
        ...
        if (profs) {
          setProfileCache((prev) => {
            ...
            for (const p of profs)
              next[p.id] = { name: p.full_name, email: p.email, avatar: p.avatar_url };
            return next;
          });
        }
      }
    ...
    if (activity.length === 0 && task) {
      supabase
        ...
        .insert({
          ...
          kind: "opened",
          body: null,
          metadata: { title: task.title, event: resolvedEvent, app } as never,
        })
        .then(() => loadActivity());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  if (!task) {
    return (
      <div className="border border-zinc-800 rounded-lg p-6 bg-zinc-900/40 text-sm text-zinc-400">
        Task <span className="font-mono text-zinc-200">{taskId}</span> not found in the template
        library.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="border border-zinc-800 rounded-xl bg-zinc-900/50 p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold leading-snug text-zinc-100">{task.title}</h1>
      ...
      <section>
        <h2 className="text-sm font-semibold text-zinc-300 mb-3">Activity</h2>
        {loading ? (
...
function ActivityItem({
  ...
  const header = (
    <span className="text-sm text-zinc-300">
      <span className="font-medium text-zinc-100">{name}</span> {kindVerb(activity.kind)}
    ...
    <li className="relative">
      ...
      {activity.kind === "commented" ? (
        <div className="border border-zinc-800 rounded-lg bg-zinc-900/40 overflow-hidden">
          <div className="px-3 py-1.5 border-b border-zinc-800 bg-zinc-900/60">{header}</div>
        ...
        <div>
          ...
          <div className="mt-1">
            <button
              onClick={() =>
                onDownload(String(activity.metadata.path ?? ""), activity.body ?? "file")
              }
